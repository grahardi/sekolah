...
<p style="font-size:13px;color:#64748b;margin:-10px 0 18px;">
    Kenaikan Gaji Berkala (KGB) ASN terjadi setiap 2 tahun sejak TMT gaji berkala terakhir.
</p>
        ...
        <table style="width:100%; border-collapse:collapse; font-size:13px;">
            <thead>
                <tr style="text-align:left; color:#64748b; font-size:11px; text-transform:uppercase; border-bottom:1px solid #f1f5f9;">
                    ...
                    <th style="padding:10px;">TMT Gaji Berkala Terakhir</th>
            ...
            <tbody>
                ...
                @php
                    $due = $p->jatuh_tempo_gaji_berkala;
                    $monthsLeft = $due ? now()->diffInMonths($due, false) : null;
                    $status = $monthsLeft === null ? null : ($monthsLeft < 0 ? 'lewat' : ($monthsLeft <= 3 ? 'dekat' : 'aman'));
                @endphp
                <tr style="border-bottom:1px solid #f8fafc; {{ $status === 'lewat' ? 'background:#fef2f2;' : ($status === 'dekat' ? 'background:#fffbeb;' : '') }}">
                    ...
                    <td style="padding:12px 10px;">{{ $p->tmt_gaji_berkala_terakhir?->format('d-m-Y') ?? '-' }}</td>
                ...
                @empty
                <tr><td colspan="5" style="padding:30px; text-align:center; color:#94a3b8;">Belum ada data TMT gaji berkala pegawai ASN.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
