...
class ExoInstanceController extends Controller
{
    ...
    public function hubungkanSekolah(Request $request, ExoInstance $exoInstance)
    {
        ...
        try {
            ...
            // Pakai hash password ASLI admin (bukan password titipan acak) -
            // identitas sama persis dgn login sekolah.co.id, admin cukup
            // pakai password yg sama utk login di Extraordinary juga.
            ...
            if (! $sudahAda) {
                $conn->table('users')->insert([
                    ...
                    'name' => $admin->name,
                    'email' => $admin->email,
                    'password' => $admin->password, // hash bcrypt asli, langsung dipakai
            ...
            $exoInstance->update(['admin_email_tersambung' => $admin->email]);

            return back()->with('success', "Instance dihubungkan ke sekolah, akun admin Extraordinary disamakan dgn login sekolah.co.id ({$admin->email}) - pakai password yg sama.");
        } catch (\Throwable $e) {
    ...
    {
        abort_unless($exoInstance->sekolah_id, 422, 'Instance belum terhubung ke sekolah manapun.');
        abort_unless($exoInstance->db_host, 422, 'Kredensial database instance belum diisi.');
        ...
        if ($siswaList->isEmpty()) {
            return back()->with('error', 'Tidak ada siswa aktif di sekolah ini.');
        }
        ...
        try {
            ...
            foreach ($siswaList->groupBy('kelas') as $kelas => $siswaSatuAngkatan) {
                ...
                foreach ($siswaSatuAngkatan->groupBy('rombel') as $rombel => $siswaSatuKelas) {
                    ...
                    foreach ($siswaSatuKelas as $siswa) {
                        ...
                        // Cocokkan agama siswa ke tabel agamas exo (buat baru kalau blm ada)
                        ...
                        // Hubungkan siswa ke SubGroup (kelasnya) - hapus dulu
                        // keanggotaan grup lama kalau pindah kelas, baru tambah yg baru.
            ...
            return back()->with('success', "Sinkron selesai: {$dibuatGrup} grup/subgrup dibuat, {$dibuatSiswa} siswa baru, {$diupdateSiswa} siswa diperbarui" . ($dilewati ? ", {$dilewati} dilewati (NIS/NISN kosong)" : '') . '.');
        } catch (\Throwable $e) {
